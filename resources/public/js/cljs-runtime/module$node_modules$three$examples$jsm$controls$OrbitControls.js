...
p){_setPrototypeOf=Object.setPrototypeOf||function(o,p){o.__proto__=p;return o};return _setPrototypeOf(o$jscomp$0,p)}function onPointerDown(event){!1!==this.enabled&&(0===this._pointers.length&&(this.domElement.setPointerCapture(event.pointerId),this.domElement.addEventListener("pointermove",this._onPointerMove),this.domElement.addEventListener("pointerup",this._onPointerUp)),this._isTrackingPointer(event)||(this._addPointer(event),"touch"===event.pointerType?this._onTouchStart(event):this._onMouseDown(event)))}
function onPointerMove(event){!1!==this.enabled&&("touch"===event.pointerType?this._onTouchMove(event):this._onMouseMove(event))}function onPointerUp(event){this._removePointer(event);switch(this._pointers.length){case 0:this.domElement.releasePointerCapture(event.pointerId);this.domElement.removeEventListener("pointermove",this._onPointerMove);this.domElement.removeEventListener("pointerup",this._onPointerUp);this.dispatchEvent(_endEvent);this.state=_STATE.NONE;break;case 1:event=this._pointers[0];
var position=this._pointerPositions[event];this._onTouchStart({pointerId:event,pageX:position.x,pageY:position.y})}}function onMouseDown(event){switch(event.button){case 0:var mouseAction=this.mouseButtons.LEFT;break;case 1:mouseAction=this.mouseButtons.MIDDLE;break;case 2:mouseAction=this.mouseButtons.RIGHT;break;default:mouseAction=-1}switch(mouseAction){case _three.MOUSE.DOLLY:if(!1===this.enableZoom)return;this._handleMouseDownDolly(event);this.state=_STATE.DOLLY;break;case _three.MOUSE.ROTATE:if(event.ctrlKey||
